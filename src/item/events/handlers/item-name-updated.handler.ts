...
import { ItemNameUpdatedEvent } from "../item-name-updated.event";

@EventsHandler(ItemUpdatedEvent)
export class ItemNameUpdatedEventHandler implements IEventHandler<ItemNameUpdatedEvent> {
    constructor(
    ...
    async handle(event: ItemNameUpdatedEvent) {
        const item = new Item();
